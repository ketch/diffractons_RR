...
def qinit(state,A,x0,y0,varx,vary,ic=1):
    ...
    if (ic==1):
        ...
        raise NotImplementedError("Unkown initial condition not implemented")
...
def my_left_BC(state,dim,t,qbc,num_ghost):
...
def psystem2D(iplot=False,kernel_language='Fortran',htmlplot=False,
    ...
    A=1.
    x0=20.0 # Center of initial perturbation
    y0=0.25 # Center of initial perturbation
    varx=5.0; vary=5.0 # Width of initial perturbation

    # Boundary conditions

    bc_x_lower=pyclaw.BC.custom;
    #bc_x_lower=pyclaw.BC.extrap 
    ...
    if restart_from_frame is not None:
        ...
        #Initial condition
        x0=10
        qinit(state,A,x0,y0,varx,vary,ic=2)
