...
def plot_p(frame,file_prefix='claw_p',path='./_output/_p',plot_slices=True,plot_pcolor=True,slices_limits=None,xshift=0.0,name='',title=True):
    ...
    if plot_pcolor:
        ...
        pl.savefig('./_plots_to_paper/counter-interaction_'+str_frame+name+'.png')
        #pl.show()                            
        pl.close()
    if plot_slices:
        ...
        pl.savefig('./_plots_to_paper/counter-interaction_'+str_frame+name+'.eps')
        pl.close()
...
if __name__== "__main__":
    ...
    ylim = get_extremum_p(15)

    print('**********************')
    print('Plotting p ...')
    plot_p(frame=0,plot_pcolor=False,slices_limits=[10,50,ylim[0]-0.05*ylim[1],1.05*ylim[1]])  
    plot_p(frame=6,plot_pcolor=False,slices_limits=[10,50,ylim[0]-0.05*ylim[1],1.05*ylim[1]])
    plot_p(frame=15,plot_pcolor=False,slices_limits=[10,50,ylim[0]-0.05*ylim[1],1.05*ylim[1]])
    plot_p(frame=15,plot_pcolor=False,slices_limits=[22.5,42.5,-0.01,0.1],name='_zoom',title=False)
    
    #plot_p(frame=0,plot_pcolor=False,slices_limits=[15,45,ylim[0]-0.05*ylim[1],1.05*ylim[1]],xshift=0)
    #plot_p(frame=6,plot_pcolor=False,slices_limits=[75,115,ylim[0]-0.05*ylim[1],1.05*ylim[1]],xshift=200)
    #plot_p(frame=15,plot_pcolor=False,slices_limits=[20,50,ylim[0]-0.05*ylim[1],1.05*ylim[1]],xshift=600)
    #plot_p(frame=15,plot_pcolor=False,slices_limits=[27.5,42.5,-0.01,0.1],xshift=600,name='_zoom',title=False)
